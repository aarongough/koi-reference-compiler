...
This is the reference parser implementation for the programming language {Koi}[http://github.com/aarongough/koi]. This is a very simple but naive compiler that does no optimization, but because of this remains very easy to understand. The compiler is implemented using the {delegation pattern}[http://en.wikipedia.org/wiki/Delegation_pattern] in that a tree representing the program is built up out of subclasses of the SyntaxNode class, each of which knows how to compile it's own specific instruction and nothing more.

=== Example

A very simple program may just push an integer onto the stack and that is all. The {Abstract Syntax Tree (AST)}[http://en.wikipedia.org/wiki/Abstract_syntax_tree] for such a program might look like this:
...
This is obviously a minimal case, but it demonstrates the principal in a straight-forward way.
